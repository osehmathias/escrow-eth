...
function App() {
    ...
    async function fetchData() {
      try {
        ...
        contract.on("Closed", async (when, event) => {
          ...
          const contractSellerBalance = await provider.getBalance(
            contractSeller
          );
          ...
          console.log(
            `${moment(
              humanReadableUnixTimestamp(when)
            ).fromNow()} - ${humanReadableUnixTimestamp(when)}`
          );
        });
  ...
  async function purchase() {
    ...
    if (typeof window.ethereum !== "undefined") {
      ...
      const signer = provider.getSigner(); // Your current metamask account;
      const forPurchase = new ethers.Contract(
        escrowAddress,
        Escrow.abi,
        signer
